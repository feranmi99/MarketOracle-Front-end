...
import { analyzeTrade, getRecentTrades } from '../../lib/api';
...
export default function DashboardPage() {
  ...
  async function loadRecentTrades() {
    try {
      ...
      const response = await getRecentTrades(token);
  ...
  async function handleAnalyze(pair: string, tradeAmount?: number, note?: string) {
    setLoading(true);
    setError(null);
    setAnalysis(null);
    try {
                    ...
                    >
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div className="space-y-4">
                          <div>
                            <p className="text-sm text-slate-500 mb-1">Trading Pair</p>
                            <p className="text-lg font-semibold text-slate-800">{analysis.metadata.pair}</p>
                          </div>
                          <div>
                            <p className="text-sm text-slate-500 mb-1">Strategy</p>
                            <p className="text-lg font-semibold text-slate-800">{analysis.metadata.strategy}</p>
                          </div>
                        </div>
                        <div className="space-y-4">
                          <div>
                            <p className="text-sm text-slate-500 mb-1">Analysis Time</p>
                            <p className="text-lg font-semibold text-slate-800">
                              {new Date(analysis.metadata.analysis_timestamp).toLocaleString()}
                            </p>
                          </div>
                          <div>
                            <p className="text-sm text-slate-500 mb-1">Trade ID</p>
